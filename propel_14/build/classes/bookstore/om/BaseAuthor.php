...
abstract class BaseAuthor extends BaseObject  implements Persistent {
	...
	/**
	 * @var        Criteria The criteria used to select the current contents of collBooks.
	 */
	private $lastBookCriteria = null;
	...
	/**
	 * Clears out the collBooks collection (array).
	 *
	...
	/**
	 * Initializes the collBooks collection (array).
	 *
	...
	public function addBook(Book $l)
	{
		...
		if (!in_array($l, $this->collBooks, true)) { // only add it if the **same** object is not already associated
			array_push($this->collBooks, $l);
			$l->setAuthor($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collBooks) {
				foreach ((array) $this->collBooks as $o) {
